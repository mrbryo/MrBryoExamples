...
function ns:ExampleFrame(parentFrame, object)
    ...
    frameLabel:SetText(("%s%s%s %s"):format(ns.example.const.colors.white, ns.L["Frame Type:"], ns.example.const.colors.ending, frameType))
    ...
    templateLabel:SetText(("%s%s%s %s"):format(ns.example.const.colors.white, ns.L["Frame Template:"], ns.example.const.colors.ending, frameTemplate))
